...
dim = \markup { \fontsize #-4 \raise #0.25 #(ly:export (ly:wide-char->utf-8 #x00b0)) }
hdim = \markup { \fontsize #-4 \raise #0.25 #(ly:export (ly:wide-char->utf-8 #x00d8)) }
aug = \markup { \fontsize #-4 \raise #0.6 "+" }
        ...
        (if (= alt FLAT)
          ...
          (make-line-markup
            (list
              ...
              (make-small-markup (make-raise-markup 0.8
                (make-text-markup "!")))
            ))
      ...
      (if (= alt 0)
        ...
        (if (= alt FLAT)
          ...
          (make-line-markup
            (list
              (make-hspace-markup 0)
              (markup #:fontsize -3 (make-raise-markup 0.5
                (make-text-markup "!")))
            ))
          ;; or handle adding the sharp symbol
          (make-line-markup
